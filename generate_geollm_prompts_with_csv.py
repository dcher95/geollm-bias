...
def get_address(lat, lon):
    ...
    response = requests.get(nominatim_url, headers = {"User-Agent": "GeoLLM"}) # User-Agent added to bypass 403 error
            ...
            if output_file:
                with open(output_file, "w") as file:
                    for i, prompt in enumerate(prompts):
                        if prompt:
                            file.write(json.dumps({"index": i, "text": prompt}) + "\n") # added index as some are dropped. easier to tie to input file
...
def main():
    ...
    coordinates_csv = "/data/cher/geollm-bias/playground/coordinates/st_louis_equal.csv"
    output_jsonl = "/data/cher/geollm-bias/playground/prompts/st_louis_equal.jsonl"
